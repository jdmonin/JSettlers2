...
public class SOCRobotClient extends SOCDisplaylessPlayerClient
{
    ...
    protected void handleROBOTDISMISS(SOCRobotDismiss mes)
    {
        ...
        if ((ga != null) && (brainQ != null))
        {
            ...
            {
                leaveGame((SOCGame) games.get(mes.getGame()), "brain not alive", true, false);
            }
        }
    }
    ...
    public void debugPrintBrainStatus(String gameName, final boolean sendTextToGame)
    {
        ...
        List rbSta = brain.debugPrintBrainStatus();
        Iterator li = rbSta.iterator();
        while (li.hasNext())
        {
            final String st = (String) li.next();
            if (sendTextToGame)
     ...
     * @param showReason  If true print bot, game, and <tt>leaveReason</tt> even if not {@link D#ebugIsEnabled()}
