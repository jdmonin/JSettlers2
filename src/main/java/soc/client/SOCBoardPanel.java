...
/*package*/ class SOCBoardPanel extends JComponent implements MouseListener, MouseMotionListener
{
        ...
        throws IllegalStateException
    {
        ...
        if (isRotated)
        {
            ...
            scaledPanelW = halfdeltaY * bh + HEXY_OFF_SLOPE_HEIGHT;
            scaledPanelH = halfdeltaX * bw;
        ...
        throws IllegalStateException
    {
        ...
        if (debugShowPotentials[2])
        {
            ...
            if (isRotated)
            {
                ...
                w = h;
                h = swapW;
                x = scaleToActual(panelMinBH - halfdeltaY) - w;
                y = 0;
            } else {
                x = 0;
                y = scaleToActual(halfdeltaY);
            }
        ...
        private void handleHover(final int x, int y, final int xb, final int yb)
        {
            ...
            if (nodePortCoord != -1)
            {
                if ((hoverMode == PLACE_INIT_SETTLEMENT) && (hoverID == nodePortCoord) && hoverIsPort)
                {
                    ...
                    if (isSeaBoard && game.isGameOptionSet(SOCGameOptionSet.K_SC_FTRI))
                    {
